...
if [ "${PHP_EXTENSIONS}" != "" ]; then
    apk --update add --no-cache --virtual .build-deps $PHPIZE_DEPS curl-dev openssl-dev gettext-dev linux-headers zlib-dev
fi
...
if [[ -z "${EXTENSIONS##*,swoole,*}" ]]; then
    ...
    isPhpVersionGreaterOrEqual 8 0
    if [[ "$?" = "1" ]]; then
        installExtensionFromTgz swoole-5.0.2 "--enable-openssl --enable-sockets --enable-swoole-curl"
